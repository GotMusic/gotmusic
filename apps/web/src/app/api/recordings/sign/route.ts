...
function createS3Client(): S3Client | null {
  ...
  if (DRIVER === "r2") {
    ...
    if (!account || !accessKeyId || !secretAccessKey) {
      console.warn("[recordings/sign] R2 credentials missing, falling back to stub");
      return null;
    }
  ...
  if (!accessKeyId || !secretAccessKey) {
    console.warn("[recordings/sign] AWS S3 credentials missing, falling back to stub");
    return null;
  }
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!client || !BUCKET) {
      console.warn("[recordings/sign] Using stub mode (httpbin)");
      return NextResponse.json({
        url: "https://httpbin.org/put",
        key: `recordings/stub-${filename}`,
        contentType,
      });
    }

    // Generate unique key: recordings/{timestamp}-{random}-{filename}
    const timestamp = Date.now();
    const random = Math.random().toString(36).slice(2, 10);
    const key = `recordings/${timestamp}-${random}-${filename}`;
    ...
    console.error("[recordings/sign] Error:", message);
