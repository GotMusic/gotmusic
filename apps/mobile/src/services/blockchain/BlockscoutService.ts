...
export interface BlockscoutServiceContextType {
  ...
  // Real-time updates
  subscribeToTransaction: (txHash: string, callback: (tx: BlockscoutTransaction) => void) => () => void;
...
class BlockscoutService {
  ...
  async getTransactionHistory(address: string, limit: number = 50): Promise<BlockscoutTransaction[]> {
    // TODO: Implement real Blockscout API call
...
export function BlockscoutServiceProvider({ children }: { children: React.ReactNode }) {
  ...
  const getTransactionHistory = async (address: string, limit?: number): Promise<BlockscoutTransaction[]> => {
    try {
      setIsLoading(true);
      setError(null);
  ...
  const getTransactionStatus = async (txHash: string): Promise<'pending' | 'success' | 'failed'> => {
    try {
      setError(null);
  ...
  const subscribeToTransaction = (txHash: string, callback: (tx: BlockscoutTransaction) => void): () => void => {
    return blockscoutService.subscribeToTransaction(txHash, callback);
  };

  const subscribeToAddress = (address: string, callback: (tx: BlockscoutTransaction) => void): () => void => {
    return blockscoutService.subscribeToAddress(address, callback);
  };

  return (
    <BlockscoutServiceContext.Provider value={{
      getTransaction,
