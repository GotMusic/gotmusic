...
export interface LitProtocolServiceContextType {
  ...
  // License Management
  checkLicense: (assetId: string, buyer: string) => Promise<{ authorized: boolean; reason?: string }>;
...
class LitProtocolService {
  ...
  async decryptAsset(encryptedData: string, encryptedSymmetricKey: string, accessControlConditions: string): Promise<LitDecryptionResult> {
    try {
      ...
      const litAccessControlConditions = accessControlConditionsArray.map((acc: LitAccessControlCondition) => ({
        contractAddress: acc.contractAddress,
        standardContractType: acc.standardContractType,
        chain: acc.chain,
        method: acc.method,
  ...
  async checkLicense(assetId: string, buyer: string): Promise<{ authorized: boolean; reason?: string }> {
    try {
      // Connect to Lit Network
      await this.litNodeClient.connect();
...
export function LitProtocolServiceProvider({ children }: PropsWithChildren<{}>) {
  ...
  const [litService] = useState(() => {
    const config: LitConfig = {
      ...
      litNetwork: (process.env.EXPO_PUBLIC_LIT_NETWORK as 'datil-dev' | 'datil-test' | 'main') || 'datil-dev',
  ...
  const decryptAsset = async (encryptedData: string, encryptedSymmetricKey: string, accessControlConditions: string): Promise<LitDecryptionResult> => {
    return await litService.decryptAsset(encryptedData, encryptedSymmetricKey, accessControlConditions);
  };

  const checkLicense = async (assetId: string, buyer: string): Promise<{ authorized: boolean; reason?: string }> => {
